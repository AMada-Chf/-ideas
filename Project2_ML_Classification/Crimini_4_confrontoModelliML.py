...
X_train, X_test, y_train, y_test = train_test_split(X_dummies, y, test_size=0.90, random_state=42)

# Crea il dizionario con i modelli
models = {
...
# Cicla su ogni modello nel dizionario
for name, model in models.items():
    # Definisci la pipeline per ogni modello
    pipeline = Pipeline([
        ('scaler', StandardScaler()),  # Standardizzazione
        (name, model)  # Modello ML
    ])
    
    # Esegui la cross-validation
    cv_scores = cross_val_score(pipeline, X_train, y_train, cv=5, scoring='accuracy')
    
    # Stampa la media delle performance ottenute
...
# Confronto delle performance con un boxplot
plt.figure(figsize=(10, 6))

# Genera il boxplot
plt.boxplot([result['cv_scores'] for result in results], labels=[result['model'] for result in results])
