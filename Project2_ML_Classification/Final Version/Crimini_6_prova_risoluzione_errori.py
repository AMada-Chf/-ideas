...
Crime2_df['Crime Category']= Crime2_df['Crime Category'].fillna('Others')
...
# Crea variabili dummy per tutte le colonne categoriche
...
# Riempimento dei valori mancanti e sostituzione in 'Weapon Category'
dataset_Nuovo_Pulito_Outliers.loc[:, 'Weapon Category'] = dataset_Nuovo_Pulito_Outliers['Weapon Category'].fillna('no Weapon')

# Riempimento dei valori mancanti e sostituzione in 'Vict Sex'
dataset_Nuovo_Pulito_Outliers.loc[:, 'Vict Sex'] = dataset_Nuovo_Pulito_Outliers['Vict Sex'].fillna('no Vict')
dataset_Nuovo_Pulito_Outliers.loc[:, 'Vict Sex'] = dataset_Nuovo_Pulito_Outliers['Vict Sex'].replace('-', 'no Vict')
dataset_Nuovo_Pulito_Outliers.loc[:, 'Vict Sex'] = dataset_Nuovo_Pulito_Outliers['Vict Sex'].replace('H', 'no Vict')

# Riempimento dei valori mancanti e sostituzione in 'Vict Descent'
dataset_Nuovo_Pulito_Outliers.loc[:, 'Vict Descent'] = dataset_Nuovo_Pulito_Outliers['Vict Descent'].fillna('no Vict')
dataset_Nuovo_Pulito_Outliers.loc[:, 'Vict Descent'] = dataset_Nuovo_Pulito_Outliers['Vict Descent'].replace('-', 'no Vict')


# suddivisione dei dati 
X_train, X_test, y_train, y_test = train_test_split(X_dummies, y, test_size=0.2, random_state=42)

#----------------------------- MODELLO ML SCELTO: LightGBM (Light Gradient Boosting Machine)

# Definisci la pipeline
pipeline = Pipeline([
    ('scaler', StandardScaler()),  # Step di standardizzazione
    ('lgbm', lgb.LGBMClassifier(random_state=42))  # Step del modello
])


# Addestra il modello finale sui dati di addestramento
pipeline.fit(X_train, y_train)

# Fai previsioni sui dati di test
y_pred = pipeline.predict(X_test)

# Esegui la cross-validation con la pipeline
